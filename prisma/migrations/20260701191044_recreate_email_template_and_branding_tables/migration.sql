-- Recreates BusinessUnit, EmailTemplate, EmailTemplateHistory, EmailBranding
-- and EmailBrandingHistory, which were unintentionally dropped by migration
-- 20260701180028_adding_bill_credentials (generated from a schema state that
-- predated the add_editable_emails feature merge into dev). All 5 tables
-- were empty at drop time on stage, so this is a safe forward-only recreate.
