...
export class OrganizationService {
  ...
  async syncOrganizationsWithDealsNEW(): Promise<Object> {
    const arrayReturn: string[] = [];
    ...
    // ---------------------------
    // 1️⃣ Buscar organizações ativas
    // ---------------------------
    const organizations = await this.prisma.organization.findMany({
      where: {
        status: 'active',
    ...
    const orgMap = new Map(organizations.map(o => [o.hubspot_id, o]));
    const organizationsId: string[] = organizations
      .map(o => o.hubspot_id)
      .filter((id): id is string => id !== null);

    // ---------------------------
    // 2️⃣ Chunk de companies
    // ---------------------------
    const chunks: string[][] = [];
    for (let i = 0; i < organizationsId.length; i += chunkSize) {
      chunks.push(organizationsId.slice(i, i + chunkSize));
    ...
    // ---------------------------
    // 3️⃣ Processar cada chunk
    // ---------------------------
    const processChunk = async (chunk: string[], index: number) => {
      try {
        // ---------------------------
        // 3.1 Associations: company → deals
        // ---------------------------
        const { data: assocData } = await axios.post(
          'https://api.hubapi.com/crm/v4/associations/company/deal/batch/read',
          { inputs: chunk.map(id => ({ id })) },
          {
            headers: {
        ...
        // ---------------------------
        // 3.2 Coletar dealIds únicos
        // ---------------------------
        const dealIds = new Set<string>();

        for (const result of assocData.results) {
          for (const assoc of result.to) {
            dealIds.add(assoc.toObjectId);
          }
        }
        ...
        for (let i = 0; i < dealIdArray.length; i += 100) {
          ...
          for (const deal of dealsData.results) {
            dealPipelineMap.set(deal.id, deal.properties.pipeline);
          }
        ...
        // ---------------------------
        // 3.4 Processar lógica de negócio
        // ---------------------------
        for (const result of assocData.results) {
          const org = orgMap.get(result.from.id);
          ...
          for (const assoc of result.to) {
            const dealHubspotId = String(assoc.toObjectId);
            const pipeline = dealPipelineMap.get(dealHubspotId);
            console.log(`Deal ${dealHubspotId} is in pipeline ${pipeline}`);
            // ❌ Ignora deals fora das pipelines válidas
            if (!pipeline || !ALLOWED_PIPELINES.includes(pipeline)) {
              //arrayReturn.push(
            ...
            if (!existingStaff) {
              ...
              */
              ...
              arrayReturn.push(
                `=> Deal ${dealHubspotId} queued for creation in ${org.name}`,
              );
              continue;
            }
            ...
            if (
              ...
              */
              ...
              arrayReturn.push(
                `=> Deal ${dealHubspotId} queued for deactivation`,
              );
            }
            ...
            if (
              ...
              */
              ...
              arrayReturn.push(
                `=> Deal ${dealHubspotId} queued for reactivation`,
              );
            }
          }
        }
