...
export class ContactFromCompanyCreationService {
  ...
  async execute(data: any, actorUserId?: string): Promise<any> {
    ...
    try {
      const response = await axios.post(
        'https://api.hubapi.com/crm/v3/objects/contacts',
        {
          properties: {
            ...
            hubspot_owner_id: data.referToUser?.id
              ? await this.getOwnerId(data.referToUser.id)
              : undefined,
            demo_owner: data.admin_id
            ...
            account_name: data.referredByAffiliate.affiliateProfile
              ? data.referredByAffiliate.affiliateProfile.full_name
              : undefined,
            account_number: data.referredByAffiliate.affiliateProfile
              ? data.referredByAffiliate.affiliateProfile.hubspot_id
              : undefined,
            alliance_commission:
              data.referredByAffiliate.affiliateProfile
                .commission_percent_default || 7,
            growth_partner_company_name: data.referredByAffiliate.organization
              ? data.referredByAffiliate.organization.name
              : undefined,
            growth_partner_email_address: data.referredByAffiliate.organization
              ? data.referredByAffiliate.organization.email
              : undefined,
            preferred_payment_method: data.referredByAffiliate.affiliateProfile
              ? data.referredByAffiliate.affiliateProfile
                  .preferred_payment_method
              : undefined,
            referral_partner: data.referredByAffiliate.affiliateProfile
              ? data.referredByAffiliate.affiliateProfile.full_name
              : undefined, //Name of the client who referred this deal.
            referral_partners_email: data.referredByAffiliate
              ? data.referredByAffiliate.email
              : undefined, //Email of the client who referred this deal.
            referral_source: 'Referral - Partner',
            referrals_industry:
              data.business_unit === 'Med Virtual' ? 'Medical' : 'Non-Medical',
            referred_to: data.referToUser?.id
              ? await this.getOwnerId(data.referToUser.id)
              : '',
          },
          // 2 Associations:
          ...
          associations: [
            ...
            data.referredByAffiliate.affiliateProfile.hubspot_id
              ? {
                  to: {
                    id: data.referredByAffiliate.affiliateProfile.hubspot_id,
                  },
                  types: [
                    {
      ...
      if (error.response) {
        console.error(
          'Error to created contact from company:',
          error.response.data,
        );
      } else {
