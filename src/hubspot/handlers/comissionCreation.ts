...
export class HandlerComissionCreation {
    ...
    async execute(event){
        try{
            ...
            if (invoiceExists?.organization?.referredByAffiliate?.affiliateProfile?.status !== 'active') {
                console.log(`The affiliate linked to the organization of the invoice with Hubspot ID ${event.objectId} is not active. No comission will be created.`);
                return;
            }
            if (invoiceExists?.invoice_amount === null) {
                console.log(`The invoice with Hubspot ID ${event.objectId} has no amount billed. No comission will be created.`);
                return;
            }
            
            let comissionAmount = invoiceExists?.organization.referredByAffiliate?.affiliateProfile?.commission_percent_default || 0;
            let baseAmmount = invoiceExists.invoice_amount;
            ...
            create: {
                ...
                affiliate_profile_id: invoiceExists.organization.referredByAffiliate.affiliateProfile.id,
            ...
            throw new BadRequestException(`Error creating comission: ${error.message}`);
        }
    }
}