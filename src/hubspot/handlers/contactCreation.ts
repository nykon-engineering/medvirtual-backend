  ...
  async execute(event) {
    ...
    try {
      const getObject = await axios.get(
        `https://api.hubapi.com/crm/v3/objects/contacts/${Number(contactId)}?properties=${properties}`,
        {
          headers: {
      ...
      if (contactExists)
        throw new BadRequestException('Contact already exists on the database');
      ...
      throw new BadRequestException(
        `Error fetching object creation Contact: ${error.message}`,
      );
    }
  }
}
