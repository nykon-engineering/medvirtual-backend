...
export class HandlerInvoicePropertyChange {
    ...
    async execute(event){
        ...
        if(event.propertyName === 'hs_invoice_status' && event.propertyValue === 'paid'){
            ...
            if (invoiceWithOrg?.organization?.id) {
                ...
                console.log(`Invoice ${event.objectId} is the first paid invoice for organization ${invoiceWithOrg.organization.id}. Checking if we need to mark the organization as deployed.`);
    ...
    private async markOrganizationDeployedFromFirstPaidInvoice(params: {
        ...
        if (!org) {
            console.log(`Organization ${params.organizationId} not found. Cannot mark as deployed.`);
            return false;
        }

        // Only referred organizations participate in the Med Alliance lifecycle.
        if (!org.referred_by_affiliate_id) {
            console.log(`Organization ${org.id} has no referred affiliate. Skipping deployed transition.`);
            return false;
        }
        ...
        if (org.med_alliance_block_reason?.startsWith('active_client_block')) {
            console.log(`Organization ${org.id} has an active-client block. Skipping deployed transition.`);
            return false;
        }
        ...
        if (org.first_paid_invoice_at) {
            console.log(`Organization ${org.id} already has first_paid_invoice_at. Skipping deployed transition.`);
            return false;
        }

        // Churned organizations should never be moved back into deployed by an invoice webhook.
        if (org.referral_stage === 'churned') {
            console.log(`Organization ${org.id} is churned. Skipping deployed transition.`);
            return false;
        }

        if (org.referral_stage === 'deployed') {
            console.log(`Organization ${org.id} is already deployed. Skipping deployed transition.`);
            return false;
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (updateResult.count === 0) {
                console.log(`Organization ${org.id} was not updated to deployed. It may have been changed by another process.`);
                return false;
            }
            ...
            console.log(`Organization ${org.id} transitioned to deployed after first paid invoice.`);
            return true;
        });
    }
}
