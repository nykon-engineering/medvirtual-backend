...
export class HandlerTicketCreation {
    ...
    async execute(event){
        ...
        try{
            const getObject = await axios.post('https://api.hubapi.com/crm/v3/objects/tickets/search',
            {
            filterGroups: [
                {
                filters: [
            ...
            if(ticketExists) throw new BadRequestException('Hire Request already exists on the database');

            const createTicket = await this.hireRequestService.create(ticketData)
            if (!createTicket) {
                throw new BadRequestException('Error creating Hire Request in the database');
            }
            return true;

        
        }catch (error) {
            throw new BadRequestException(`Error fetching object on creation HR: ${error.message}`);
        }
