      ...
      if (specificField) {
        switch (specificField) {
          case 'assign_user_id':
            if (data.assign_user_id) {
              ...
              if (Array.isArray(data.assign_user_id)) {
                ...
                const userIds = data.assign_user_id
                  .split(',')
          ...
          case 'assign_sourcing_id':
            hubspotProperties.pairing_specialist = data.assign_sourcing_id
              ? await this.getOwnerId(data.assign_sourcing_id)
              : undefined;
            break;

          case 'assign_staffing_coordinator':
            hubspotProperties.staffing_coordinator =
              data.assign_staffing_coordinator
                ? await this.getOwnerId(data.assign_staffing_coordinator)
                : undefined;
            break;

          case 'closed_date':
            hubspotProperties.closed_date = formatDateForCA(
              new Date(),
              'America/Los_Angeles',
            );
            break;
          ...
          case 'cancel_date':
            hubspotProperties.ticket_cancel_date = formatDateForCA(
              new Date(),
              'America/Los_Angeles',
            );
            break;
        ...
        if (data.assign_sourcing_id) {
          hubspotProperties.pairing_specialist = await this.getOwnerId(
            data.assign_sourcing_id,
          );
        }

        if (data.assign_staffing_coordinator) {
          hubspotProperties.staffing_coordinator = await this.getOwnerId(
            data.assign_staffing_coordinator,
          );
        }

        hubspotProperties.pairing_session_conducted =
          data.pairing_session_conducted;
        hubspotProperties.pairing_outcome_reason =
          data.pairing_session_outcome_reason;
        hubspotProperties.count_of_candidates_invited_ =
          data.count_of_candidates_invited_ || undefined;
        hubspotProperties.count_of_candidates_attended_ =
          data.count_of_candidates_attended_ || undefined;
        hubspotProperties.count_of_candidates_interviewed_ =
          data.count_of_candidates_interviewed_ || undefined;
        hubspotProperties.client_signed_contract =
          data.client_signed_contract || undefined;
        hubspotProperties.client_signed_contract_closing_ticket =
