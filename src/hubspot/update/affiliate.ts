...
export class AffiliateUpdateService {
    ...
    async deactivate(hubspotId: string, actorUserId?: string, entityId?: string): Promise<void> {
    ...
    async reactivate(hubspotId: string, actorUserId?: string, entityId?: string): Promise<void> {
        const source = actorUserId ? HubspotAuditSource.user_action : HubspotAuditSource.cron;
        try {
            await axios.patch(
                `https://api.hubapi.com/crm/v3/objects/${OBJECT_TYPE}/${Number(hubspotId)}`,
    ...
    async updateCommission(hubspotId: string, commissionPercent: number, actorUserId?: string, entityId?: string): Promise<void> {
        const source = actorUserId ? HubspotAuditSource.user_action : HubspotAuditSource.cron;
        try {
            await axios.patch(
                `https://api.hubapi.com/crm/v3/objects/${OBJECT_TYPE}/${Number(hubspotId)}`,
    ...
    async updateBankingData(hubspotId: string, accountName: string, accountNumber: string, actorUserId?: string, entityId?: string): Promise<void> {
        const source = actorUserId ? HubspotAuditSource.user_action : HubspotAuditSource.cron;
        try {
            await axios.patch(
                `https://api.hubapi.com/crm/v3/objects/${OBJECT_TYPE}/${Number(hubspotId)}`,
                { properties: { account_name: accountName, account_number: accountNumber } },
                {
                    headers: {
                        Authorization: `Bearer ${process.env.HUBSPOT_ACCESS_TOKEN}`,
                        "Content-Type": "application/json",
    ...
    async clearBankingData(hubspotId: string, actorUserId?: string, entityId?: string): Promise<void> {
        const source = actorUserId ? HubspotAuditSource.user_action : HubspotAuditSource.cron;
        try {
            await axios.patch(
                `https://api.hubapi.com/crm/v3/objects/${OBJECT_TYPE}/${Number(hubspotId)}`,
