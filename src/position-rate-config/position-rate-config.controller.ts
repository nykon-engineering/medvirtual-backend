...
export class PositionRateConfigController {
  ...
  @Get()
  @ApiOperation({ summary: 'List all VA position rate configurations with optional search and pagination' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'perPage', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by position name' })
  @ApiResponse({ status: 200, description: 'Position rate configs retrieved successfully' })
  ...
  @Put(':position')
  @ApiOperation({ summary: 'Create or update the rate configuration for a specific VA position' })
  @ApiParam({ name: 'position', description: 'VA position name', example: 'Medical Virtual Assistant' })
  @ApiBody({ type: UpdatePositionRateConfigDto })
  @ApiResponse({ status: 200, description: 'Position rate config created or updated successfully' })
