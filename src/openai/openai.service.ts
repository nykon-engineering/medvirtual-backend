...
export class OpenaiService {
    ...
    /**
     * Generates a concise 1–2 sentence summary of a candidate's bio.
     * Used to populate description_summary for list/card views where space is limited.
     * Only called when the bio exceeds the minimum character threshold (300 chars).
     */
    async summarizeCandidateBio(bio: string): Promise<string> {
        const apiKey = process.env.OPENAI_API_KEY;
        if (!apiKey) {
            throw new BadRequestException('OPENAI_API_KEY is not defined in environment variables');
        }
        ...
        const prompt = `
You are a concise summarization assistant for a healthcare staffing platform.

You will receive a professional bio of a Virtual Assistant (VA) candidate in the healthcare industry.

Your task is to write a **short, punchy summary of 1 to 2 sentences** that captures:
1. The candidate's primary area of expertise or specialization
2. Their most relevant professional strength or value proposition for a healthcare client

### Rules
- Write in third person (e.g. "Experienced VA specializing in...", "Healthcare professional with...")
- Be factual: only use information present in the bio — do not invent or embellish
- Do not include names, contact details, or personally identifiable information
- Output only the summary text — no labels, no JSON, no markdown, no bullet points
- Maximum 2 sentences; aim for under 200 characters total
        ...
        try {
            const response = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a concise summarization assistant. Return only the summary text, no extra formatting.',
                    },
                    {
                        role: 'user',
                        content: prompt,
            ...
            if (error?.type === 'insufficient_quota') {
                console.error('[OpenAI] summarizeCandidateBio — insufficient quota');
                throw new BadRequestException('You don\'t have credits. Check your plan/billing.');
            }
            if (error?.type === 'rate_limit_error') {
                console.error('[OpenAI] summarizeCandidateBio — rate limit exceeded');
                throw new BadRequestException('Rate limit exceeded. Please try again later.');
            }
            console.error('[OpenAI] summarizeCandidateBio — unexpected error:', error);
            throw new BadRequestException('Failed to generate candidate bio summary.');
        }
    }
